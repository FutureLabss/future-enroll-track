...
BEGIN
  ...
  first_due AS (
    SELECT inv.enrollment_id, MIN(inst.due_date)::date AS enrollment_date
    FROM public.invoices inv
    JOIN public.installments inst ON inst.invoice_id = inv.id
    GROUP BY inv.enrollment_id
  ),
  rev AS (
    SELECT date_trunc('month', fd.enrollment_date)::date AS m,
           COALESCE(SUM(e.amount_paid), 0) AS total
    FROM public.enrollments e
    JOIN first_due fd ON fd.enrollment_id = e.id
