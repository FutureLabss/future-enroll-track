-- Rewrite get_finance_summary to scope all data to get_my_hub_id().
-- Previously the function queried across all hubs with no isolation.
-- Also switches revenue source from installments.due_date to payments.created_at
-- so hubs that don't use installments (e.g. RhemaHub demo) show correct figures.
