-- ── 1. payroll_runs: add hub-admin RLS (only superadmin existed before) ────────
  ...
  USING (
    ...
    OR (public.has_role(auth.uid(), 'admin'::app_role)
        AND EXISTS (
          ...
          WHERE s.id = payroll_runs.staff_id
            AND s.hub_id = public.get_my_hub_id()
        ))
  )
  WITH CHECK (
    public.is_superadmin()
...
-- ── 2. get_finance_summary: use due_date for installments ──────────────────────
-- paid_at was set to the backfill date (Apr/May 2026), not the real payment date.
-- due_date correctly reflects the historical payment schedule (Nov 2025–Jun 2026).
...
BEGIN
  ...
  rev AS (
    SELECT date_trunc('month', paid_at)::date AS m,
           SUM(amount)                        AS total
    FROM (
      -- RhemaHub: payments table (Paystack), use created_at as payment date
      ...
      UNION ALL
      -- FutureLabs: installments table, use due_date as the revenue month
      -- (paid_at reflects when data was backfilled, not the actual payment month)
