-- The previous migration set search_path to the quoted string
-- 'public, extensions' which PostgreSQL treats as a single schema name
-- rather than two separate schemas, dropping public from the path and
-- breaking the app_role type lookup.
--
-- Fix: restore search_path = public and qualify gen_random_bytes with
-- the extensions schema explicitly instead.
